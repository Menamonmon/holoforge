...
async def test_fixed_point_slow_dot(dut):
    ...
    for t in range(test_count):
        ...
        assert out == P, f"({A_vec}) * ({B_vec}) = {P} != {out}"
...
def main():
    ...
    sources = [proj_path / "src" / "hdl" / "common" / "fixed_point_slow_dot.sv"]
    ...
    for params in gen_params():
        ...
        runner.build(
            sources=sources,
            hdl_toplevel="fixed_point_slow_dot",
            always=True,
        ...
        runner.test(
            hdl_toplevel="fixed_point_slow_dot",
            test_module="test_fixed_point_slow_dot",

...
@cocotb.test()
async def test_test_test(dut):
	"""Test simple projection case."""
	# Initialize Clock
	cocotb.start_soon(Clock(dut.clk_100mhz, 10, units="ns").start())  # 100MHz clock
...
def main():
	...
	sources = [
		...
		proj_path / "src" / "hdl" / "common" / "video_sig_gen.sv",
		proj_path / "src" / "hdl" / "common" / "evt_counter.sv",
		proj_path
		/ "src"
		/ "hdl"
		/ "common"

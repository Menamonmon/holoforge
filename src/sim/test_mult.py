...
async def test_mult(dut):
	...
	for t in range(test_count):
		...
		A_val = generate_twos_complement_random(A_size) 
		B_val = generate_twos_complement_random(B_size) 
		# max and min numbers
		# 1
		# 1 + 4 = 5
		# 5 + 9 = 14
		...
		dut.A.value = BinaryValue(get_twos_complement(A_val,A_size), n_bits=A_size,bigEndian=False,binaryRepresentation=BinaryRepresentation.TWOS_COMPLEMENT
		)
		dut.B.value = BinaryValue(get_twos_complement(B_val,B_size), n_bits=B_size,bigEndian=False,binaryRepresentation=BinaryRepresentation.TWOS_COMPLEMENT
		)
		await ClockCycles(dut.clk_in, 3)
		P_size = A_size + B_size - P_extra_frac_bits
		P = A_val * B_val
		print(dut.P.value.signed_integer)
		print(P>>P_extra_frac_bits)
		assert dut.P.value.signed_integer == P>>P_extra_frac_bits
...
def main():
	...
	sources = [proj_path / "hdl" / "common" / "fixed_point_mult.sv"]
